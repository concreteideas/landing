(() => {
  ...
  Promise.all([fetch('../data/projects.json'), fetch('../data/products.json')])
    .then(async ([projectResponse, productResponse]) => {
      if (!projectResponse.ok || !productResponse.ok) throw new Error('Unable to load project');
      const [projects, products] = await Promise.all([projectResponse.json(), productResponse.json()]);
      ...
      detail.innerHTML = `
        ...
        <div class="project-detail__content"><p class="project-kicker">${project.category}</p><h1>${project.title}</h1><p class="project-detail__overview">${project.description}</p>
