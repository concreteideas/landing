(() => {
  ...
  const render = () => {
    ...
    root.innerHTML = `<div class="enquiry-layout"><div class="enquiry-list"><div class="enquiry-list__heading"><div><p class="site-eyebrow">Selected pieces</p><h2>${items.length} ${items.length === 1 ? 'piece' : 'pieces'}</h2></div><button class="enquiry-clear" type="button" data-enquiry-action="clear">Clear list</button></div>${items.map((item) => `<article class="enquiry-item"><a class="enquiry-item__image" href="${productUrl(item.id)}"><img src="${item.image || '../brand/icon.png'}" alt="${escapeHtml(item.name)}" loading="lazy"></a><div class="enquiry-item__info"><p class="enquiry-item__eyebrow">${escapeHtml(item.sizeName || 'Standard')}</p><h3><a href="${productUrl(item.id)}">${escapeHtml(item.name)}</a></h3><p class="enquiry-item__dimensions"><strong>Dimension:</strong> ${escapeHtml(item.dimensions || '')}<br><strong>Weight:</strong> ${escapeHtml(item.weight || 'To be confirmed')}</p><button class="enquiry-remove" type="button" data-enquiry-action="remove" data-key="${escapeHtml(cart.itemKey(item))}">Remove</button></div><div class="enquiry-quantity" aria-label="Quantity for ${escapeHtml(item.name)}, ${escapeHtml(item.sizeName || 'Standard')}"><button type="button" data-enquiry-action="decrease" data-key="${escapeHtml(cart.itemKey(item))}" aria-label="Decrease quantity">−</button><span>${item.quantity}</span><button type="button" data-enquiry-action="increase" data-key="${escapeHtml(cart.itemKey(item))}" aria-label="Increase quantity">+</button></div></article>`).join('')}</div><aside class="enquiry-summary"><p class="site-eyebrow">Ready to talk?</p><h2>Send this selection to the studio.</h2><p>We'll review your requirements and get back to you with availability, pricing and delivery guidance.</p><a class="site-button site-button--primary enquiry-summary__button" href="../contact/index.html?enquiry=1">Send enquiry <span aria-hidden="true">→</span></a><a class="enquiry-summary__continue" href="../products/index.html">Continue browsing</a></aside></div>`;
  };
